...
const NavTabs = ({
  ...
  const scrollTabs = (direction: "left" | "right") => {
    ...
    const delta = Math.max(160, el.clientWidth * 0.65);
    el.scrollBy({
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-full w-full items-center gap-2 px-4 sm:px-5 md:px-6 2xl:max-w-360 lg:max-w-[1280px]">
        <button
          type="button"
          aria-label="Scroll tabs left"
          onClick={() => scrollTabs("left")}
          disabled={!canScrollLeft}
          className="inline-flex h-11 w-11 flex-none items-center justify-center rounded-full text-[#111827] transition-colors duration-200 hover:bg-black/5 disabled:pointer-events-none disabled:opacity-0 md:hidden"
        >
        ...
        <div
          ref={scrollRef}
          className="min-w-0 flex-1 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden md:overflow-visible"
        >
          <TabsList
